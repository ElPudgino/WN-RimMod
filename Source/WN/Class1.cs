...
namespace WN
{
    ...
    public static class WN_DefOf
    {
        ...
        public static IncidentDef Advent_WN;
        public static IncidentDef WN_Escape;
        public static IncidentDef PlagueDoctorArrival;
    ...
    */
    ...
    public class IncidentWorker_WN_Advent : IncidentWorker
    {
    ...
    public class IncidentWorker_WN_Escape : IncidentWorker
    {
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
    ...
    public class CompApostleSpear : CompApostle
    {
        ...
        {
            (parent as Pawn).jobs.ClearQueuedJobs();
            ...
            foreach (IntVec3 c in list)
            {
                if (!c.CloseToEdge(map,6) && (c - pawn.Position).LengthHorizontalSquared <= (closestcell - pawn.Position).LengthHorizontal)
                {
                    closestcell = c;
                }
            }
            ...
            if (!closestcell.CloseToEdge(map, 4))
            {
    ...
    {
        [DebugAction("General", "Advent", false, false, false, false, false, 0, false, actionType = DebugActionType.ToolMap, allowedGameStates = AllowedGameStates.PlayingOnMap)]
        private static void Debug_Advent()
        {
            foreach (Thing thing in Find.CurrentMap.thingGrid.ThingsAt(UI.MouseCell()).ToList<Thing>())
            {
                if (thing is PlagueDoctor)
                {
                    Advent(thing as PlagueDoctor,null);
                }
            }
        ...
        public static void Advent(PlagueDoctor pd, WhiteNight whitenight)
        {
            ...
            //FiringIncident inc = new FiringIncident(WN_DefOf.Advent_WN,Find.Storyteller.storytellerComp);
            ...
            if (whitenight == null)
            {
                ...
                wn = (Pawn)GenSpawn.Spawn(wn, pd.PositionHeld, mp, (WipeMode)0);
                ...
                WN_DefOf.Advent_WN.Worker.TryExecute(new IncidentParms());
            }
            ...
            else 
            {
                ...
                WN_DefOf.WN_Escape.Worker.TryExecute(new IncidentParms());
            }
    ...
    public class CompWhiteNight : ThingComp
    {
        ...
        public override void Notify_Killed(Map prevMap, DamageInfo? dinfo = null)
        {
            ...
            if (dropego)
            {               
                Thing ego = ThingMaker.MakeThing(WN_DefOf.ParadiseLostWeapon);
    ...
    */
    ...
    public class Ring : Thing
    {
        ...
        private List<trg_dist> hitThings;
        public override void SpawnSetup(Map map, bool respawningAfterLoad)
        {
            base.SpawnSetup(map, respawningAfterLoad);
            ...
            hitThings = new List<trg_dist>();

            for (int x = -maxRadius;x<= maxRadius;x++)
            {
                for (int z = -maxRadius; z <= maxRadius; z++)
                {
                    IntVec3 c = new IntVec3(x,0,z) + this.Position;
                    if (c.InBounds(map))
                    {
                        ...
                        for (int j = 0; j < thingList.Count; ++j)
                        {
                            ...
                            if (thing is Pawn)
                            {
                                try
                                {
                                    hitThings.Add(new trg_dist(thing as Pawn, Position.DistanceTo(thing.Position)));
                                }
                                catch { }
        ...
        {
            for (int i = hitThings.Count -1; i >= 0 ;i--)
            {
                if (hitThings[i].dist <= radius)
                {
                    OnHit(hitThings[i].trg);
                    hitThings.RemoveAt(i);
                }
            }
        ...
        private struct trg_dist
        {
            public Pawn trg;
            public float dist;

            public trg_dist(Pawn t,float d)
            {
                trg = t; dist = d;
            }
        }
        ...
        private void OnHit(Thing thing)
        {
            if (thing != null && thing is Pawn hitPawn)
            {
                ...
                if (hitPawn.RaceProps.IsMechanoid)
                {
                    mech=true;
                }
    ...
    [HarmonyPatch]   
    public static class PatchCorpses
    {
    ...
    [HarmonyPatch]
    public static class PatchPawnTakeDamage
    {
        [HarmonyPatch(typeof(Pawn), "PreApplyDamage")]
        [HarmonyPrefix]
        public static bool PawnPrefix(Pawn __instance, ref DamageInfo dinfo, out bool absorbed)
        {
    ...
    public static class HarmonyPatches
    {
        ...
        public static Harmony harmony = new Harmony(id: "doge.LobCoWhiteNight");
